"""
This file imports all needed shapefiles and their components for users to choose the reference system of their choice. 
...
def get_shapes(file_path):
    ...
    """
    This function returns the geometries for the different geographical boundaries

    Required Arguments:
    
    1) file_path (String) - The path to the shapefiles

    Returns: The geometries of the geographical boundaries from the shapefile
    """
...
def download_shape_files():
    ...
    """
    This function downloads the shapefiles and then moves the shapefiles into their respective folder. 
...
def import_shapefiles(file_path, line_color, boundary_type):
    ...
    if boundary_type == 'cwa':
        ...
        if os.path.exists(f"NWS CWA Boundaries/w_05mr24.shp"):
            ...
            extract_zipped_files(f"NWS CWA Boundaries/w_05mr24.zip", f"NWS CWA Boundaries")
        ...
        if os.path.exists(f"NWS Fire Weather Zones/fz05mr24.shp"):
            ...
            extract_zipped_files(f"NWS Fire Weather Zones/fz05mr24.zip", f"NWS Fire Weather Zones")
        ...
        if os.path.exists(f"NWS Public Zones/z_05mr24.shp"):
            ...
            extract_zipped_files(f"NWS Public Zones/z_05mr24.zip", f"NWS Public Zones")
